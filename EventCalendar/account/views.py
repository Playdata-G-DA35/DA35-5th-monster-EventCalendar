...
def user_detail(request):
    ...
    if user.team:
        ...
        team = None
    return render(request, "account/detail.html", {"object":user, "team":team})
...
def user_delete(request):
    # 정보삭제 -> logout
    request.user.delete()
    # 일반적인 항목의 데이터를 DB에서 삭제 -> 삭제할 정보의 ID(path/request parameter) 받아서 select 한 뒤에 삭제
    # item = Item.object.get(pk=pk)
    # item.delete()
...
@login_required
def user_delete2(request, pk): #path parameter로 삭제할 Data의 pk를 받기.
    # 조회해서 삭제
